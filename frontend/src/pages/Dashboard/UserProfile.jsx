...
export default function UserProfile() {
  ...
  const handleUserAction = async (actionType) => {
    ...
    if (actionType === "Follow") {
      ...
      endpoint = `/users/${userId}/block`; // You’ll need to implement this backend route
    } else if (actionType === "Report") {
      endpoint = `/users/${userId}/report`; // You’ll need to implement this backend route
    }
    ...
    try {
      ...
      alert(data.message || `${actionType} successful`);
    } catch (err) {
      console.error(`❌ ${actionType} error:`, err);
      alert(`Failed to ${actionType.toLowerCase()}: ${err.message}`);
    }
  };
        ...
        <div className="flex justify-center gap-5 mb-6">
          {["Follow", "Block", "Report"].map((btn, i) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => handleUserAction(btn, user.uid)}
